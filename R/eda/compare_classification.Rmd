...
`r length(list.files(here::here("data", "classifier", "images"), pattern = ".*",  recursive = TRUE))` images currently available, using both a trained neural network and optical character recognition with `tesseract`.

In the output below, blue text designates hyperlinks that should point to the relevant image in my Dropbox storage.

```{r}
source(here::here("R", "image", "utilities.R"))
...
```
I opted to record results separately, since I planned to record CNN results many times. The datasets can be joined using the `image` column as a key.
...
A contingency table shows the different classifications, with the neural network 
on the rows and OCR on the columns.
Most agree, with the exception of the network predicting "Trump" far too often. 
...
```
The proportion of cases where one or both classifiers was wrong `r mean(disagreements)`. 
...
Here, I randomly sample 10 sets of words and 
display them beside the name of the associated image.
