...
for (x in c("house", "senate")) {
  ...
  for (i in seq(nrow(cong[[x]]))) {
    if (!file.exists(
      here(
        fp, paste0(
          "wayback_timemap_", file_pattern(cong[[x]], i, var = "url"), ".txt"
        )
      )
    )) {
      tryCatch(
...
## Check number of memento files per chamber
for (x in c("house", "senate")) {
  out <- list.files(
    here("data", "raw", "wayback", x, "timestamp_donation"),
    pattern = ".txt"
  )
  target <- seq(nrow(cong[[x]])) %>% 
    map_chr(
      ~ paste0(
        "wayback_timemap_", file_pattern(cong[[x]], .x, var = "url"), ".txt"
      )
    )
  assert_that(identical(sort(out), sort(target)))
}
...
for (x in c("house", "senate")) {
  ...
  merged_df <- wayback_merge(
    wayback_donate, cong[[x]] %>% select(-matches("^year$")),
    var = "url"
  ) %>%
