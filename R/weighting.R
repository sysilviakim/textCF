...
house_dem_notrump <- house_unique[house_unique$word_trump == 0 &
  house_unique$party == "DEMOCRAT", ]
assert_that(nrow(house_unique) == sum(
  nrow(house_gop_notrump),
...
houseplot <- as.data.frame(rbind(
  house_gop_trump, house_gop_notrump,
...
senate_dem_notrump <- senate_unique[senate_unique$word_trump == 0 &
  senate_unique$party == "DEMOCRAT", ]
assert_that(nrow(senate_unique) == sum(
  nrow(senate_gop_notrump),
...
senateplot <- as.data.frame(rbind(
  senate_gop_trump, senate_gop_notrump,
