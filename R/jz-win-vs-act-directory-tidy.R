...
# Get text from ActBlue
c1 <- corpus(actblue_federal %>% select(
...
# WC = word count BUT we checked that this counts punctuation too
# (unless punct. is removed from the corpus)
...
(
  F1 <- winred_Lexicoder %>%
    ...
    mutate(index = row_number()) %>%
    select(
      index, Dic, anger, anticipation, disgust, fear, joy,
    ...
    ggplot(
      aes(
        y = fct_reorder(factor(index), value),
        x = value, color = name, fill = name
      )
    ) +
    geom_col() +
    ...
    labs(
      x = "Proportion of (classified) words",
      y = "", title = "Language analysis summary of WinRed ads",
      color = "", fill = ""
    ) +
...
(
  F2 <- actblue_Lexicoder %>%
    ...
    labs(
      x = "Proportion of (classified) words",
      y = "", title = "Language analysis summary of Actblue ads",
      color = "", fill = ""
    ) +
    theme(axis.text.y = element_blank())
)
...
BOTH_Lexicoder <- actblue_Lexicoder %>%
  mutate(source = "Actblue") %>%
  bind_rows(winred_Lexicoder %>% mutate(source = "WinRed")) %>%
  mutate(
    anger_fear_disgust_negative = (anger + fear + disgust + negative) / Dic
  )
